# -*- coding: utf-8 -*-
"""
下载《中国近岸养殖政策 中文文献清单》32 篇论文 PDF（v1，中文版）。
- 解析 Markdown 清单：提取 DOI 与所有链接（期刊官网/SciEngine/知网等）
- 下载策略（按优先级）：
    1) Unpaywall 查 OA，遍历全部 OA 镜像源（url_for_pdf / pdf_url / url）
    2) DOI 直达 https://doi.org/<doi>
    3) 条目自带链接（期刊官网 / SciEngine / 期刊 XML 页面等）
    4) 对 HTML 落地页自动搜索 .pdf 链接再尝试（SciEngine、xml-journal 等 OA 站点）
- requests + verify=False + 完整请求头，规避 SSL/403 部分问题
- 结果写入 UTF-8 文件，避免控制台 GBK 崩溃
- 知网（CNKI）文献需账号登录，脚本无法下载时会如实标记"需人工/知网"

用法：py -3 download_zh_papers.py            # 全部
      py -3 download_zh_papers.py 1,2,3     # 仅下载指定序号（测试用）
"""
...
SRC = os.path.join(BASE, "中国近岸养殖政策_文献收集_中文.md")
OUT = os.path.join(BASE, "papers", "中文")
REPORT = os.path.join(BASE, "下载结果_中国近岸养殖政策_中文.md")
...
for e in entries:
    ...
    if done:
        ...
        joined = " ".join(cands and [c[1] for c in cands] or [])
        rec["status"] = "need_manual" if ("cnki" in joined or "wanfang" in joined or "xuewen" in joined) else "paywalled_or_failed"
    results.append(rec)
...
manual = [r for r in results if r["status"] == "need_manual"]
fail = [r for r in results if r["status"] not in ("downloaded", "need_manual")]

rep = []
rep.append("# 下载结果汇总（中国近岸养殖政策·中文文献）\n")
rep.append("成功下载 **%d** 篇 ｜ 需人工/知网 **%d** 篇 ｜ 未获取 **%d** 篇 ｜ 共 **%d** 篇。\n" % (len(ok), len(manual), len(fail), len(results)))
rep.append("\n## 已下载\n")
for r in ok:
...
print("\n===== 完成：成功 %d / 需人工 %d / 未获取 %d / 共 %d =====" % (len(ok), len(manual), len(fail), len(results)))
print("PDF 目录:", OUT)
print("报告:", REPORT)
